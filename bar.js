                ...
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = 0; j < (multi || 1); j++) {
                        ...
                        if (j == multi - 1) {
                            ...
                            L = paper.text(bars[i * (multi || 1) + j].x, y + height - barvgutter / 2, label).attr(chartinst.txtattr).attr({ fill: opts.legendcolor || "#000"}).insertBefore(covers[i * (multi || 1) + j]);
if (rotate) {
				L.rotate(90);
			}
                            var bb = L.getBBox();
                ...
                for (var i = 0; i < len; i++) {
                    for (var j = 0; j < (multi || 1); j++) {
                      // did not remove the loop because don't yet know whether to accept multi array input for arrays
                        ...
                        L = paper.text(bars[i * (multi || 1) + j].x, isBottom ? y + height - barvgutter / 2 : bars[i * (multi || 1) + j].y - 10, label).attr(chartinst.txtattr).attr({ fill: opts.legendcolor || "#000"}).insertBefore(covers[i * (multi || 1) + j]);
			if (rotate) {
				L.rotate(90);
